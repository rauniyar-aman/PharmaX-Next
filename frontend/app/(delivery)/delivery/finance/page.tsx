...
import type { DeliveryFinanceProfile } from '@/types'
...
export default function DeliveryFinancePage() {
  const [profile, setProfile] = useState<DeliveryFinanceProfile | null>(null)
  const [loading, setLoading] = useState(true)
  const [tab, setTab] = useState<(typeof TABS)[number]['key']>('EARNINGS')

  useEffect(() => {
    api.get('/delivery/finance/').then((r) => setProfile(r.data.data))
      ...
      {loading ? (
        ...
        earnings.length === 0 ? (
          <div className="bg-surface rounded-2xl border border-outline-variant py-16 text-center text-on-surface-variant">
            ...
            <p className="mt-2 text-sm">No earnings yet — completed deliveries will show up here.</p>
          </div>
        ) : (
            ...
            <div className="divide-y divide-outline-variant">
              {earnings.map((e) => (
                <div key={e.id} className="grid grid-cols-2 sm:grid-cols-[1fr_1fr_1fr] gap-2 px-4 py-3 text-sm">
                  <div>
                    <p className="text-xs font-mono text-on-surface-variant">#{e.order_id.slice(0, 8).toUpperCase()}</p>
          ...
          <div className="divide-y divide-outline-variant">
            {liabilities.map((l) => (
              <div key={l.id} className="grid grid-cols-2 sm:grid-cols-[1fr_1fr_1.4fr] gap-2 px-4 py-3 text-sm">
                <div>
                  <p className="text-xs font-mono text-on-surface-variant">#{l.order_id.slice(0, 8).toUpperCase()}</p>
