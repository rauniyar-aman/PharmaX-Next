...
export default function AddBrandPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      await api.post('/admin/brands/', { name, manufacturer: manufacturer || undefined, description, logo_url: logoUrl || undefined })
      toast.success('Brand added!')
      router.push('/admin/brands')
    } catch (err: any) {
      const data = err.response?.data
      toast.error(data?.errors ? Object.values(data.errors).flat().join(', ') : data?.message || 'Failed to add brand.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="max-w-md space-y-5">
      <div className="flex items-center gap-2 text-sm text-on-surface-variant">
        ...
        <span className="text-on-surface font-medium">Add Brand</span>
      </div>
      <h1 className="text-2xl font-bold text-on-surface">Add New Brand</h1>
      <form onSubmit={handleSubmit} className="bg-surface rounded-2xl border border-outline-variant p-5 space-y-4">
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={saving}
            ...
            {saving ? <><div className="w-4 h-4 border-2 border-on-primary border-t-transparent rounded-full animate-spin" />Saving...</> : 'Add Brand'}
          </button>
