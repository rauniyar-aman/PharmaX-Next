...
export default function AdminDashboardPage() {
  ...
  const cards = [
    ...
    { label: 'Pending Orders', value: stats?.pending_orders ?? 0, icon: 'pending_actions', color: 'bg-amber-50 text-amber-600', href: '/admin/orders' },
              ...
              {stats.recent_orders.slice(0, 6).map((order) => (
                <Link key={order.id} href={`/admin/orders`}
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-on-surface">{order.user?.full_name || 'Customer'}</p>
                  </div>
                  <div className="text-right">
                    <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${STATUS_COLORS[order.status] || 'bg-surface-container text-on-surface-variant'}`}>
