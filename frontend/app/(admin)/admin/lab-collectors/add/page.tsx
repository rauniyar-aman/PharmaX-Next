...
export default function AddLabCollectorPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      await api.post('/admin/lab-collectors/', form)
      toast.success('Lab collector created! Verify them before they can accept collections.')
      router.push('/admin/lab-collectors')
    } catch (err: any) {
      const data = err.response?.data
      toast.error(data?.errors ? Object.values(data.errors).flat().join(', ') : data?.message || 'Failed to create lab collector.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="max-w-2xl space-y-5">
      <div className="flex items-center gap-2 text-sm text-on-surface-variant">
        <Link href="/admin/lab-collectors" className="hover:text-primary transition-colors">Lab Collectors</Link>
        <span className="material-symbols-outlined" style={{ fontSize: '16px' }}>chevron_right</span>
        <span className="text-on-surface font-medium">Add Collector</span>
      </div>
      <h1 className="text-2xl font-bold text-on-surface">Add New Lab Collector</h1>
      <p className="text-sm text-on-surface-variant">
        Creates a login account for the collector. It starts unverified — verify it from the list once you've confirmed their details.
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={saving}
            ...
            {saving ? <><div className="w-4 h-4 border-2 border-on-primary border-t-transparent rounded-full animate-spin" />Saving...</> : 'Create Lab Collector'}
          </button>
          <Link href="/admin/lab-collectors" className="px-6 py-2.5 border border-outline-variant text-on-surface-variant text-sm font-semibold rounded-xl hover:bg-surface-container transition-colors">
            Cancel
          </Link>
        </div>
      </form>
