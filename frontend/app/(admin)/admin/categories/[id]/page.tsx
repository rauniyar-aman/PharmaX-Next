...
export default function EditCategoryPage() {
  ...
  useEffect(() => {
    api.get(`/admin/categories/${id}/`).then((r) => {
      const c = r.data.data.category
      setName(c.name || '')
      setDescription(c.description || '')
      setIcon(c.icon || '')
    }).catch(() => toast.error('Failed to load category.')).finally(() => setLoading(false))
  }, [id])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      await api.put(`/admin/categories/${id}/`, { name, description, icon: icon || null })
  ...
  return (
    <div className="max-w-md space-y-5">
      ...
      <h1 className="text-2xl font-bold text-on-surface">Edit Category</h1>
