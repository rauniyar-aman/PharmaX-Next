...
import type { Permission, AdminUser } from '@/types'
...
export default function EditAdminPage() {
  ...
  useEffect(() => {
    if (!user?.is_super_admin || !id) return
    Promise.all([
      api.get('/admin/permissions/'),
      api.get(`/admin/admins/${id}/`),
    ]).then(([permRes, adminRes]) => {
      setPermissions(permRes.data.data.permissions || [])
      const a: AdminUser = adminRes.data.data.admin
      setAdmin(a)
      setFullName(a.full_name)
      setIsActive(a.is_active)
      setIsSuperAdmin(a.is_super_admin)
      setSelectedCodes(new Set(a.permission_codes))
    }).catch(() => toast.error('Failed to load admin.')).finally(() => setLoading(false))
  }, [user, id])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      await api.patch(`/admin/admins/${id}/`, {
        full_name: fullName,
        is_active: isActive,
      ...
      toast.error(data?.errors ? Object.values(data.errors).flat().join(', ') : data?.message || 'Failed to update admin.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="max-w-2xl space-y-5">
      ...
      <h1 className="text-2xl font-bold text-on-surface">Edit Admin</h1>
        ...
        <div className="bg-surface rounded-2xl border border-outline-variant p-5 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="email" disabled value={admin.email}
                className="mt-1 w-full px-3 py-2.5 border border-outline-variant rounded-xl bg-surface-container-low text-sm text-on-surface-variant" />
            </div>
          </div>
