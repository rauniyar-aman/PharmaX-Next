...
import type { CollectorCodLiability } from '@/types'
...
function CollectorCodLiabilitiesContent() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const user = useAuthStore((s) => s.user)
  const [liabilities, setLiabilities] = useState<CollectorCodLiability[]>([])
  const [loading, setLoading] = useState(true)
  const [statusFilter, setStatusFilter] = useState(searchParams.get('status') || '')
  const [confirmingFor, setConfirmingFor] = useState<CollectorCodLiability | null>(null)
  ...
  const load = useCallback(() => {
    ...
    api.get('/admin/finance/collector-cod-liabilities/', { params })
  ...
  const openConfirm = (l: CollectorCodLiability) => {
    setConfirmingFor(l)
  ...
  const submitConfirm = async () => {
    ...
    setSubmitting(true)
    try {
      const res = await api.post(`/admin/finance/collector-cod-liabilities/${confirmingFor.id}/confirm-remittance/`, {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold text-on-surface">Lab Collector COD Liabilities</h1>
        <p className="text-sm text-on-surface-variant mt-1">
          Cash lab collectors collected at the doorstep and still owe back to the platform — the opposite direction from collector earnings.
        </p>
      </div>
      ...
      {/* Distinct red/error accent throughout — same "money owed TO the platform" framing as
          the delivery agent COD liabilities page. */}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-error/5 border-b border-error/20">
              <tr>
                {['Collector', 'Booking', 'Amount Collected', 'Days Outstanding', 'Status', 'Remitted Via', ''].map((h) => (
            ...
            <tbody className="divide-y divide-outline-variant">
              {loading ? (
                ...
                <tr key={l.id} className="hover:bg-error/5 transition-colors">
                  <td className="px-4 py-3 font-medium text-on-surface whitespace-nowrap">{l.collector_name}</td>
                  <td className="px-4 py-3 text-on-surface-variant text-xs">{l.lab_test_name}</td>
          ...
          <div className="bg-surface rounded-2xl border border-outline-variant p-6 w-full max-w-sm space-y-4" onClick={(e) => e.stopPropagation()}>
            <div>
              ...
              <p className="text-xs text-on-surface-variant mt-1">
                {confirmingFor.collector_name} · {fmt(confirmingFor.amount_collected)}
              </p>
            </div>
            <div>
