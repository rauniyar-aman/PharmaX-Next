...
const REASON_MESSAGES: Record<string, string> = {
  ...
  order_not_found: 'We could not find the order associated with this payment.',
  missing_data: 'The payment gateway did not return the expected data.',
  bad_data: 'The payment gateway returned an unreadable response.',
}

function PaymentFailedContent() {
  const searchParams = useSearchParams()
  const reason = searchParams.get('reason')
  const message = (reason && REASON_MESSAGES[reason]) || 'Something went wrong with your payment. Your order was not placed.'
