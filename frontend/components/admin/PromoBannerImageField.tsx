...
export default function PromoBannerImageField({ value, onChange }: Props) {
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await api.post('/admin/promo-banners/upload-image/', formData, {
  ...
  return (
    <div>
      <label className="text-xs font-medium text-on-surface-variant">Image (optional — falls back to gradient + icon if not set)</label>
