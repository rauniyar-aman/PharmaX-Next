...
interface DeliveryRequestsStore {
  ...
  lastArrival: { items: DeliveryFulfillment[]; at: number } | null
...
/** Single source of truth for the rider's available-deliveries poll — same pattern as
 * usePharmacyRequestsStore, started once from (delivery)/layout.tsx so it runs across every
 * delivery page (Active, not just Requests), not just while that one page happens to be open. */
export const useDeliveryRequestsStore = create<DeliveryRequestsStore>((set, get) => ({
  requests: [],
  loading: true,
  lastArrival: null,
  _knownIds: null,
  ...
  refetch: async () => {
    try {
      ...
      const requests: DeliveryFulfillment[] = res.data.data.requests || []
