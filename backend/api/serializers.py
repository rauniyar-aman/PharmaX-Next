...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    lab_collector_verified = serializers.SerializerMethodField()
    lab_collector_online = serializers.SerializerMethodField()
    ...
    def get_lab_collector_verified(self, obj):
        if obj.role != 'LAB_COLLECTOR':
            return None
        collector = getattr(obj, 'lab_collector', None)
        return collector.is_verified if collector else False

    def get_lab_collector_online(self, obj):
        if obj.role != 'LAB_COLLECTOR':
            return None
        collector = getattr(obj, 'lab_collector', None)
        return collector.is_online if collector else False
    ...
    def to_representation(self, instance):
        ...
        if instance.role != 'LAB_COLLECTOR':
            data.pop('lab_collector_verified', None)
            data.pop('lab_collector_online', None)
        return data
